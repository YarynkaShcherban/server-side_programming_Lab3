...
def main():
    ...
    position1 = repo.positions.model.objects.create(
        role="Store Manager", salary=Decimal("2500.00"))
    position2 = repo.positions.model.objects.create(
        role="Sales Assistant", salary=Decimal("1200.00"))
    position3 = repo.positions.model.objects.create(
        role="Inventory Specialist", salary=Decimal("1800.00"))
    position4 = repo.positions.model.objects.create(
        role="Customer Support", salary=Decimal("1500.00"))

    employee_data = [
        ("John", "Doe", date(1985, 4, 12), "+1-614-555-1234",
         "john.doe@example.com", position1),
        ("Emily", "Smith", date(1990, 7, 22), "+1-212-555-5678",
         "emily.smith@example.com", position2),
        ("Michael", "Johnson", date(1988, 2, 5), "+44-20-555-7890",
         "michael.johnson@example.co.uk", position3),
        ("Sophie", "Brown", date(1995, 11, 30), "+1-503-555-3456",
         "sophie.brown@example.com", position4),
    ]
    ...
    for fn, ln, email, phone in clients_data:
        clients.append(repo.clients.model.objects.create(
            first_name=fn, last_name=ln, email=email, phone=phone))

    genres_names = ["Fiction", "Mystery", "Classic",
                    "Adventure", "Dystopian", "Fantasy", "Self-Help"]
    genres = [repo.genres.model.objects.create(
        name=gname) for gname in genres_names]
    ...
    purchase1 = repo.purchases.model.objects.create(
        client=clients[0],
        store=store1,
        employee=repo.employees.model.objects.get(
